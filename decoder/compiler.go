...
func (self _Instr) formatSwitchLabels() string {
	var i int
	var v int
	var m []string = make([]string, 0, len(self.vs()))
...
func (self *_Compiler) compileOps(p *_Program, sp int, vt reflect.Type) {
	switch vt.Kind() {
	case reflect.Bool:
		self.compilePrimitive(p, _OP_bool)
	case reflect.Int:
		self.compilePrimitive(p, _OP_int())
	case reflect.Int8:
		self.compilePrimitive(p, _OP_i8)
	case reflect.Int16:
		self.compilePrimitive(p, _OP_i16)
	case reflect.Int32:
		self.compilePrimitive(p, _OP_i32)
	case reflect.Int64:
		self.compilePrimitive(p, _OP_i64)
	case reflect.Uint:
		self.compilePrimitive(p, _OP_uint())
	case reflect.Uint8:
		self.compilePrimitive(p, _OP_u8)
	case reflect.Uint16:
		self.compilePrimitive(p, _OP_u16)
	case reflect.Uint32:
		self.compilePrimitive(p, _OP_u32)
	case reflect.Uint64:
		self.compilePrimitive(p, _OP_u64)
	case reflect.Uintptr:
		self.compilePrimitive(p, _OP_uintptr())
	case reflect.Float32:
		self.compilePrimitive(p, _OP_f32)
	case reflect.Float64:
		self.compilePrimitive(p, _OP_f64)
	case reflect.String:
