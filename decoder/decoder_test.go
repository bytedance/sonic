...
import (
    ...
    sonicutf8 `github.com/bytedance/sonic/internal/utf8`
...
func TestDecoder_InvalidUtf8(t *testing.T) {
    ...
    for _, test := range tests {
        spew.Dump(test)
        {
            ...
            d.ValidateString()
            serr := d.Decode(&sgot)
            jerr := json.Unmarshal([]byte(test.input), &jgot)
            assert.Equal(t, test.hasError,serr != nil, test.name)
            assert.Equal(t, test.hasError, jerr != nil, test.name)
            assert.Equal(t, test.expect, sgot, test.name)
            assert.Equal(t, test.expect, jgot, test.name)
        }
        // check validutf8
        {
            got, err := sonicutf8.CorrectWith(nil, []byte(test.input), "\ufffd")
