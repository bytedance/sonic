...
func (self *Value) SetByPath(val Value, allowAppend bool, path ...interface{}) (bool, error) {
	...
	for i, k := range path {
		if id, ok := k.(int); ok && id >= 0 {
			if _, err = p.searchIndex(id); err != 0 {
				...
				if !allowAppend {
					return false, ErrIndexOutOfRange{false, i}
				}
				idx = i
				break
			}
	...
	if err == 0 {
		...
		var err error
		b, err = appendPathValue(b, idx, path[idx:], allowAppend, val)
		if err != nil {
			return exist, err
		}
	}
...
// [2,"a"],1 => {"a":1}
// ["a",2],1  => "a":[1]
func appendPathValue(b []byte, start int, path []interface{}, allowAppend bool, val Value) ([]byte, error) {
	for i, k := range path {
		...
		if _, ok := n.(int); ok {
			if !allowAppend {
				return nil, ErrIndexOutOfRange{true, start + i + 1}
			}
			b = append(b, "["...)
