...
func (d *structDecoder) FromDom(vp unsafe.Pointer, node internal.Node, ctx *context) error {
	...
	next := obj.Children()
	for i := 0; i < obj.Len(); i++ {
		key, _ := internal.NewNode(next).AsStrRef(ctx)
...
//go:noinline
func (d *structDecoder) wrapper_error(err error, idx int) error {
	if e, ok := err.(*MismatchTypeError); ok {
		e.Struct = d.structName
		e.Field  = d.fields[idx].Name
		return e
	}
	return err
}
