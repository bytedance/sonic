...
var _OpFuncTab = [256]func(*Assembler, *ir.Instr){
	...
	ir.OP_is_nil_or_empty_slice: (*Assembler)._asm_OP_is_nil_or_empty_slice,
...
func (self *Assembler) _asm_OP_is_nil_or_empty_slice(p *ir.Instr) {
	self.Emit("BTQ", jit.Imm(int64(alg.BitNoOmitZeroSlice)), _ARG_fv)
	self.Sjmp("JC", "_is_nil_slice_ptr_{n}")
	self.Emit("CMPQ", jit.Ptr(_SP_p, 8), jit.Imm(0)) // CMPQ 8(SP.p), $0
	self.Xjmp("JE", p.Vi())                          // JE   p.Vi()
	self.Sjmp("JMP", "_is_nil_slice_end_{n}")
	self.Link("_is_nil_slice_ptr_{n}")
	self.Emit("CMPQ", jit.Ptr(_SP_p, 0), jit.Imm(0)) // CMPQ (SP.p), $0
	self.Xjmp("JE", p.Vi())                          // JE   p.Vi()
	self.Link("_is_nil_slice_end_{n}")
}
