...
var _OpFuncTab = [256]func(*Assembler, *ir.Instr){
	...
	ir.OP_if_omit_all_empty_or_but_slice: (*Assembler)._asm_OP_if_omit_all_empty_or_but_slice,
...
func (self *Assembler) _asm_OP_if_omit_all_empty_or_but_slice(p *ir.Instr) {
	self.Emit("MOVQ", _ARG_fv, _AX)
	self.Emit("BTQ", jit.Imm(alg.BitOmitAllEmpty), _AX)
	self.Sjmp("JC", "_omit_all_empty_enabled_{n}")
	self.Emit("BTQ", jit.Imm(alg.BitOmitAllEmptyButSlice), _AX)
	self.Xjmp("JNC", p.Vi())
	self.Link("_omit_all_empty_enabled_{n}")
}
