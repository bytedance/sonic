...
func TestIssue827_AllTextMarshalerTypes(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			sonicResult := string(sonicData)
			
			sonicStdData, err := sonic.ConfigStd.Marshal(tc.data)
			require.NoError(t, err)
			sonicStdResult := string(sonicStdData)
			
			var sonicMap, jsonMap map[string]int
			require.NoError(t, json.Unmarshal([]byte(sonicResult), &sonicMap))
			require.NoError(t, json.Unmarshal([]byte(expected), &jsonMap))
			require.Equal(t, jsonMap, sonicMap)
			
			require.Equal(t, expected, sonicStdResult)
		})
	}
}
