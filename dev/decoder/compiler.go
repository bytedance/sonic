...
func (c *compiler) compileMap(vt reflect.Type) decFunc {
	...
	if mt.Key.Kind() == reflect.String {
		if canFast {
			return &mapStrKeyFastDecoder{
				mapType: mt,
				elemDec: c.compile(vt.Elem()),
			}
		} else {
			return &mapStrKeyStdDecoder{
				mapType: mt,
				elemDec: c.compile(vt.Elem()),
			}
		}
	...
	if mt.Key.IsInt64() {
		if canFast {
			return &mapI64KeyFastDecoder{
				mapType: mt,
				elemDec: c.compile(vt.Elem()),
			}
		} else {
			return &mapI64KeyStdDecoder{
				mapType: mt,
				elemDec: c.compile(vt.Elem()),
			}
		}
	...
	if mt.Key.IsInt32() {
		if canFast {
			return &mapI32KeyFastDecoder{
				mapType: mt,
				elemDec: c.compile(vt.Elem()),
			}
		} else {
			return &mapI32KeyStdDecoder{
				mapType: mt,
				elemDec: c.compile(vt.Elem()),
			}
		}
	...
	if mt.Key.IsUint64() {
		if canFast {
			return &mapU64KeyFastDecoder{
				mapType: mt,
				elemDec: c.compile(vt.Elem()),
			}
		} else {
			return &mapU64KeyStdDecoder{
				mapType: mt,
				elemDec: c.compile(vt.Elem()),
			}
		}
	...
	if mt.Key.IsUint32() {
		if canFast {
			return &mapU32KeyFastDecoder{
				mapType: mt,
				elemDec: c.compile(vt.Elem()),
			}
		} else {
			return &mapU32KeyStdDecoder{
				mapType: mt,
				elemDec: c.compile(vt.Elem()),
			}
		}
