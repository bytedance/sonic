...
func TestUnmarshal(t *testing.T) {
	for i, tt := range unmarshalTests {
		...
		require.Equal(t, tt.out, v.Elem().Interface(), tt)
...
func TestEmptyString(t *testing.T) {
	...
	data := `{"Number1":"", "Number2":"","Pass":"true"}`
	var t2, t3 T2
	t2.Number2 = "a"
	t3.Number2 = "a"
...
// 			data:        `{"a":` + strings.Repeat(`[`, 1) + `0` + strings.Repeat(`]`, 1) + `}`,
// 			errMaxDepth: false,
// 		},
// 		{
...
func testDecodeInvalidUtf8(t *testing.T, data []byte) {
	...
	serr := sonic.ConfigStd.Unmarshal(data, &sgot)
