...
INLINE_ALL long skip_positive_1(const GoString *src, long *p) {
    long i = *p - 1;
    long r = do_skip_number(src->buf + i, src->len - i);
...
INLINE_ALL long skip_number_1(const GoString *src, long *p) {
    const char* ss = src->buf;
...
INLINE_ALL long skip_one_1(const GoString *src, long *p, StateMachine *m, uint64_t flags) {
    fsm_init(m, FSM_VAL);
