...
static always_inline svuint8_t simd256_shr(const svuint8_t input, const int shift) {
    svuint16_t shifted = svlsr_u16_z(svptrue_b16(), svreinterpret_u16_u8(input), svreinterpret_u16_u8(svdup_u8(shift)));
    return svreinterpret_u8_u16(svand_u16_z(svptrue_b16(), shifted, svdup_u16(0xFF >> shift)));
}
...
static always_inline svuint8_t simd256_lookup16(const svuint8_t input, const uint8_t* table) {
 ...
 //
  ...
  static always_inline svuint8_t check_special_cases(const svuint8_t input, const svuint8_t prev1) {
    ...
    static always_inline bool check_error(svuint8_t* err) {
        svbool_t result = svcmpgt_n_u8(svptrue_b8(), *err, 0);
        return svptest_first(svptrue_b8(), result);
    }

    static always_inline void check64_utf(svuint8_t* err, svuint8_t* in, svuint8_t* incom, const uint8_t* start) {
        svuint8_t input = svld1_u8(svptrue_b8(), start);
        svuint8_t input2 = svld1_u8(svptrue_b8(), start + 32);
        // check utf-8 chars
        svuint8_t error1 = check_utf8_bytes(input, *in);
        svuint8_t error2 = check_utf8_bytes(input2, input);
        svuint8_t tmp_err = svorr_u8_z(svptrue_b8(), *err, svorr_u8_z(svptrue_b8(), error1, error2));
        *err = tmp_err;
        *in = input2;
        *incom = is_incomplete(input2);
    }

    static always_inline void check64(svuint8_t* err, svuint8_t *in, svuint8_t *incom, const uint8_t* start) {
        // fast path for contiguous ASCII
        svuint8_t input = svld1_u8(svptrue_b8(), start);
        svuint8_t input2 = svld1_u8(svptrue_b8(), start + 32);
        ...
        // check utf-8
        if (likely(is_ascii(reducer))) {
            svuint8_t tmp_err = svorr_u8_z(svptrue_b8(), *err, *incom);
            *err = tmp_err;
            return;
        }
        check64_utf(err, in, incom, start);
    }

    static always_inline void check128(svuint8_t* err, svuint8_t *in, svuint8_t *incom, const uint8_t* start) {
        // fast path for contiguous ASCII
        svuint8_t input = svld1_u8(svptrue_b8(), start);
        svuint8_t input2 = svld1_u8(svptrue_b8(), start + 32);
        ...
        if (likely(is_ascii(reducer1))) {
            svuint8_t tmp_err = svorr_u8_z(svptrue_b8(), *err, *incom);
            *err = tmp_err;
            check64_utf(err, in, incom, start + 64);
            return;
        }
    ...
    static always_inline void check_eof(svuint8_t* err, svuint8_t *incom) {
        svuint8_t tmp_err = svorr_u8_z(svptrue_b8(), *err, *incom);
        *err = tmp_err;
    }

    static always_inline void check_remain(svuint8_t *err, svuint8_t *in, svuint8_t *incom, const uint8_t* start, const uint8_t* end) {
        uint8_t buffer[64] = {0};
        int i = 0;
        while (start < end) {
            buffer[i++] = *(start++);
        };
        check64(err, in, incom, buffer);
        check_eof(err, incom);
    }
    ...
    static always_inline long validate_utf8_sve(const GoString* s) {
        ...
        svuint8_t error = svdup_u8(0x0);
        svuint8_t prev_input_block = svdup_u8(0x0);
        svuint8_t prev_incomplete = svdup_u8(0x0);

        while (start < (end - 128)) {
            check128(&error, &prev_input_block, &prev_incomplete, start);
            if (check_error(&error)) {
            }
            start += 128;
        };
        while (start < end - 64) {
            check64(&error, &prev_input_block, &prev_incomplete, start);
            start += 64;
        }
        check_remain(&error, &prev_input_block, &prev_incomplete, start, end);
        return check_error(&error) ? -1 : 0;
    }
