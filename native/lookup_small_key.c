...
static always_inline void to_lower(uint8_t* dst, const uint8_t* src, size_t len) {
    ...
    while (len >= 32){
        ...
        __m256i lt = _mm256_cmplt_epi8(op, Z_);
...
static always_inline size_t key_buf_size(GoString* key) {
    return key->len + 1;
}
...
long lookup_small_key(GoString* key, GoSlice* table, long lower_off) {
    ...
    if (len <= 8) {
        for (int i = 0; i < cnt; i++) {
            ...
            xprintf("compare with %g\n", &gs);
            if (compre_8(p, kp, len) == 0) {
                xprintf("compare %g with %g got indx is %d\n", key, &gs,  p[len]);
                return p[len];
            } else {
                p += len + 1;
            }
    ...
    if (len <= 8) {
        for (int i = 0; i < cnt; i++) {
            ...
            GoString kl = {
             ...
             xprintf("compare %g with lower %g  forr offset %d\n", &kl, &gs, offset);
            if (compre_8(p, lower, len) == 0) {
                  xprintf("compare %g with lower %g got indx is %d\n", &kl, &gs,  p[len]);
                return p[len];
            } else {
                p += len + 1;
            }
