...
static always_inline bool node_on_num(void* ctx, size_t pos, uint64_t typ, num num) {
    ...
    buf->stat.number++;
    buf->cur++;
    xprintf("on number end, pos is %d\n", pos);
    node_buf_grow(buf, 1);
    return true;
}
...
static always_inline uint64_t* node_on_container_start(void* ctx, size_t pos, bool is_obj) {
    ...
    xprintf("store parent pre is %x \n", buf->parent);
    buf->parent = buf->cur - buf->start;
    buf->cur++;
    buf->depth++;
          xprintf("addr cur %x \n", &(buf->cur));
      xprintf("addr is start is %x\n", &(buf->start));
       xprintf("addr end is %x\n", &(buf->end));
...
static always_inline uint64_t* node_on_container_end(void* ctx, size_t len) {
    xprintf("on container end, len is %d", len);
    node_buf* buf = (node_buf*)ctx;
...
static always_inline uint64_t get_nonspace_bits(const uint8_t* s) {
    ...
    xprintf("los s is '%s', lo mask is %d\n", (char*)s, mask_lo);
...
static always_inline char skip_space(nonspace_block* slf, reader* rdr) {
    char c = skip_space_impl(slf, rdr);
    xprintf("skip space c is %c\n", c);
    return c;
}
// Parser/ParseString -------------------------------------------------------
...
static always_inline bool has_backslash(string_block* block) {
    xprintf("bs is %x quote is %x\n", block->bs, block->quote);
...
static always_inline long parse_string_inplace(uint8_t** cur, bool* has_esc, bool disable_surrogates_error) {
    GoString gs = { (char*)*cur, 100 };
    xprintf("parse string: %g @ ptr %x \n", &gs, (char*)(*cur));
    string_block block;
    uint8_t* start = *cur;
    v256u v;
    ...
    while (true) {
        block = string_block_new(*cur);
            xprintf("block is %d %d %d\n", block.bs, block.esc, block.quote);
...
escape:
    esc = *(*cur + 1);
    xprintf("escaped char is %c\n", esc);
    if (likely(esc) != 'u') {
        if (unlikely(ESCAPED_TAB[esc]) == 0) {
            xprintf("invalid escaped ");
    ...
    if (**cur == '0') {
        *cur += 1;
        ...
        if (likely(**cur != 'e' && **cur != 'E')) { // parse integer
            ...
            if (unlikely(exp == 1)) {
                uint64_t res = 0;
                if (__builtin_mul_overflow(sig, 10, &res) || __builtin_add_overflow(res, *(*cur - 1) , &res)) {
                    goto parse_float;
                }
                if (neg) {
...
static always_inline error_code parse(GoParser* slf, reader* rdr, visitor* vis) {
  ...
  xprintf(" parent is %d continue object state lenght is %d type is %d\n", node_buf_parent(nodes)->typ, get_count(*state), node_buf_top(nodes)->typ & TYPE_MASK);
        switch (node_buf_top(nodes)->typ & TYPE_MASK) {
            ...
            default: {
                if (get_type(node_buf_parent(nodes)->typ) == OBJECT) {
                    ...
                    xprintf("continue object state lenght is %d\n", get_count(*state));
                        c = skip_space(&slf->nbk, rdr);
                        goto arr_cont;
                }
            }
...
obj_key:
    ...
    xprintf("visit on key is %g pos is %d\n", &gs,  pos);
...
long parse_with_padding(void* p) {
  ...
  xprintf("n is is %d\n", n);
    xprintf("cur node ptr is %x parent is %d\n", n->cur, n->parent);
    visitor vis = {
