...
int64_t GetSPAdjust(const MCInst &Inst, uint64_t PC,
  ...
  uint64_t LocalVL = VL;
  if (LocalVL == 0) {
    std::ifstream SveVLFile("/proc/sys/abi/sve_default_vector_length");
    if (SveVLFile.is_open()) {
      std::string VLStr;
      if (std::getline(SveVLFile, VLStr)) {
        LocalVL = std::stoull(VLStr);
        SveVLFile.close();
      }
    }
  }
  if (StartWith(Mnem, "addvl") || StartWith(Mnem, "addpl")) {
    assert((LocalVL != 0) &&
           "SVE vector length isn't set and can't be read from "
           "/proc/sys/abi/sve_default_vector_length, but addvl and addpl are "
           "used to adjust sp.");
    Def = Bytes * LocalVL;
...
void CalcSPDelta(tool::mc::MCContextBundle &Bundle,
  ...
  for (size_t b = 0; b < BasicBlocks.size(); b++) {
    ...
    for (size_t i = BB.FirstIdx; i <= BB.LastIdx; i++) {
      Cur += GetSPAdjust(Text[i], TextPC[i], Bundle, SPDelta, VL);
      if (Cur < MinCur) {
        MinCur = Cur;
      }
    }
