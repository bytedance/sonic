...
func isZeroTyped(p unsafe.Pointer, vt *rt.GoType) bool {
	switch vt.Kind() {
	case reflect.Map:
		return (*(**rt.GoMap)(p)).Count == 0
	case reflect.Slice:
		return (*rt.GoSlice)(p).Len == 0
	case reflect.String:
		return (*rt.GoString)(p).Len == 0
	case reflect.Struct:
		return isZeroStruct(p, vt)
	case reflect.Interface:
		return (*rt.GoEface)(p).Value == nil
	default:
		return false
	}
}
...
func isTrivialZeroable(vt reflect.Type) bool {
	...
	case reflect.Slice:
		return false
	case reflect.Struct:
		return isStructTrivialZeroable(vt)
	default:
		return false
	}
}
...
func isStructTrivialZeroable(vt reflect.Type) bool {
	for i := 0; i < vt.NumField(); i++ {
		if !isTrivialZeroable(vt.Field(i).Type) {
			return false
		}
	}
	return true
