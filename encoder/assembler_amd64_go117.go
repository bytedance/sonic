...
func (self *_Assembler) save_state() {
    ...
    self.Emit("CMPQ", _R9, jit.Imm(int64(option.MaxEncodeStackSize)*_StateSize))        // CMPQ R9, $_StackLimit
    self.Sjmp("JA"  , _LB_error_too_deep)               // JA   _error_too_deep
    self.Emit("MOVQ", _R9, jit.Ptr(_ST, 0))             // MOVQ R9, (ST)
    self.Emit("MOVQ", jit.Ptr(_ST, 8), _R9)             // MOVQ 8(ST), R9 
    self.Emit("MOVQ", _SP_x, jit.Sib(_R9, _CX, 1, 0))   // MOVQ SP.x, 0(ST)(CX)
    self.Emit("MOVQ", _SP_f, jit.Sib(_R9, _CX, 1, 8))  // MOVQ SP.f, 8(ST)(CX)
    self.WriteRecNotAX(0, _SP_p, jit.Sib(_R9, _CX, 1, 16))  // MOVQ SP.p, 16(ST)(CX)
    self.WriteRecNotAX(1, _SP_q, jit.Sib(_R9, _CX, 1, 24))  // MOVQ SP.q, 24(ST)(CX)
}
