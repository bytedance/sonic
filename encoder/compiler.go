...
func _OP_is_zero_ints() _Op {
	switch _INT_SIZE {
	case 32:
		return _OP_is_zero_4
	case 64:
		return _OP_is_zero_8
	default:
...
func (self _Instr) isBranch() bool {
	...
	case _OP_is_zero_8:
		fallthrough
	case _OP_is_zero_mem:
		fallthrough
	case _OP_is_zero_safe:
		fallthrough
	case _OP_map_check_key:
		fallthrough
	case _OP_slice_next:
		fallthrough
	case _OP_cond_testc:
...
func (self _Instr) disassemble() string {
	...
	case _OP_marshal_p:
		fallthrough
	case _OP_marshal_text:
		fallthrough
	case _OP_marshal_text_p:
...
func (self *_Compiler) compileStructFieldZero(p *_Program, vt reflect.Type) {
	...
	case reflect.Interface:
		p.add(_OP_is_nil_p1)
	case reflect.Map:
		p.add(_OP_is_zero_map)
	case reflect.Ptr:
		p.add(_OP_is_nil)
	case reflect.Slice:
		p.add(_OP_is_nil_p1)
	case reflect.Struct:
		self.compileStructFieldNonTrivialZero(p, vt)
	default:
		panic(error_type(vt))
	}
}
